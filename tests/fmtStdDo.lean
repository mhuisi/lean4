...
`⇓ _ => _` and `⇓? _ => _` (`fmtNoThrowPostCond`, `fmtMayThrowPostCond`), `EStack⟨_⟩` and
`estack⟨_⟩` (`fmtEStackType`, `fmtEStackValue`), the complete lattice notations `⌜_⌝`, `⨅ _, _` and
...
variable (σ : Type) (handleIO handleUser : String → σ → Prop) (handleParse : ParseError → σ → Prop)

/-! ## `EStack⟨_⟩` and `estack⟨_⟩` -/

example : Type := EStack⟨⟩

example : Type := EStack⟨String → σ → Prop⟩

example : Type := EStack⟨String → σ → Prop, Unit → σ → Prop⟩

example : Type :=
  EStack⟨String → σ → Prop, ParseError → σ → Prop, Unit → σ → Prop, IO.Error → σ → Prop⟩

example : Type :=
  EStack⟨String → σ → Prop, ParseError → σ → Prop, Unit → σ → Prop, IO.Error → σ → Prop,
    Nat → σ → Prop⟩

example : Type := EStack⟨String → EStack⟨Unit → σ → Prop⟩ → Prop, Unit → σ → Prop⟩

example : EStack⟨⟩ := estack⟨⟩

example : EStack⟨String → σ → Prop⟩ := estack⟨handleIO⟩

example : EStack⟨String → σ → Prop, String → σ → Prop⟩ := estack⟨handleIO, handleUser⟩

example : EStack⟨String → σ → Prop, String → σ → Prop, ParseError → σ → Prop⟩ :=
  estack⟨fun message => handleIO message, fun message => handleUser message, handleParse⟩

example : EStack⟨String → σ → Prop, String → σ → Prop, ParseError → σ → Prop⟩ :=
  estack⟨fun message state => handleIO message state ∧ message ≠ "",
    fun message state => handleUser message state ∧ message ≠ "",
    fun error state => handleParse error state ∧ error.line ≠ 0⟩
