...
/-!
Tests that the body atom of a doc comment keeps the whitespace after the opening delimiter. Doc
strings are extracted without this whitespace, and the pretty printer puts one space after the
opening delimiter.
-/
...
/-- Shows the atoms of the first doc comment in the command `src`. -/
def showDocAtoms (src : String) : CommandElabM Unit := do
  ...
  logInfo m!"{repr doc[0].getAtomVal} {repr doc[1].getAtomVal}"

def ppCommandSrc (src : String) : CommandElabM Unit := do
  logInfo (← liftCoreM <| PrettyPrinter.ppCommand ⟨← parseCommand src⟩)

/-! The body atom starts right after the opening delimiter. -/
