...
- `full d` — forbid any text after `d` on the same line. `free d` — resolve `d` on its own and hide its width, height, and cost from the surrounding document (used for comments, which must not influence how the code around them breaks).
