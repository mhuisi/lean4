...
notations (a `ParserDescr` of shape `symbol >> term:argPrec` resp. a `TrailingParserDescr` of
shape `symbol` at `lhsPrec == prec`) get `fmtPrefixOperator` / `fmtPostfixOperator`. Only write a
formatter for these when the derived one is wrong.
...
2. Format an individual file with
   ...
   **Elaboration errors do not block formatting** — only parse errors, header errors, and
   ...
   e.g. to derive an operator's fixity and precedences from its `ParserDescr`), so the file's
