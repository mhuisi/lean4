...
public import Lean.Data.Fmt.Formatters
...
def filterAlreadyFormattedComments
  ...
  let comments := comments.map fun _ cs =>
    cs.filter fun c => Id.run do
      let some fw := formattedWhitespace.get? c.originalTokenRange
        | return true
      match c.originalWhitespaceKind with
      | .leading =>
        return fw.formattedLeadingRanges.all (! ·.includes c.originalWhitespaceRange)
      | .trailing =>
        return fw.formattedTrailingRanges.all (! ·.includes c.originalWhitespaceRange)
...
public def main (env : Environment) (opts : Options) (stx : Syntax) : Except Error String := do
  ...
  let comments ← collectComments lineInfos stx
  let (taggedDoc, syntaxToTags, formattedWhitespace, _) ← FmtM.run env opts lineInfos <| fmt stx
  let comments := filterAlreadyFormattedComments comments formattedWhitespace
