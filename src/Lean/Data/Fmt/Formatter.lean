...
structure Measure (τ : Type) where
  ...
  /--
  Level of identation of the last line of the rendering represented by this measure.
  Used by the `unindented (unindentToLineIndentation := true)` formatting document.
  -/
...
def Measure.append (m1 m2 : Measure τ) : Measure τ where
  ...
  lastLineIndentation := m2.lastLineIndentation
  output := do
    m1.output
    m2.output

/-- Adjusts the indentation, non-cumulative indentation and last line indentation of a measure. -/
def Measure.adjustIndentation (m : Measure τ)
    (newIndentation : Nat)
...
inductive TaintedMeasure (τ : Type) where
  ...
  | adjustTaintedIndentation (tm : TaintedMeasure τ)
    (newIndentation newNonCumulativeIndentation : Nat)
    (maxNewlineCount? : Option Nat)
  /--
...
-/
def MeasureSet.adjustIndentation (m : MeasureSet τ)
    (newIndentation : Nat)
    (newNonCumulativeIndentation : Nat) : MeasureSet τ :=
  match m with
  | .set ms =>
    .set <| ms.map (·.adjustIndentation
      newIndentation
      newNonCumulativeIndentation)
  | .tainted tm => .tainted
    (.adjustTaintedIndentation
      tm
      newIndentation
      newNonCumulativeIndentation
      tm.maxNewlineCount?)
  ...
  fun d columnPos indentation nonCumulativeIndentation lastLineIndentation fullness => do
    ...
    | .newline .. =>
      ...
      return .set [{
        ...
        lastLineIndentation := lineIndentation
