...
inductive Doc.Emptiness where
  | alwaysEmpty
  | alwaysEmptyIfFlattened
  | sometimesNonEmpty

def Doc.Emptiness.max (e1 e2 : Doc.Emptiness) : Doc.Emptiness :=
  let v
    | .alwaysEmpty => 0
...
inductive Doc where
  ...
  | aligned (d : Doc)
  /--
  Sets the current level of indentation within an inner document to 0 if `unindentToLineIndentation`
  is `false` or to the level of indentation of the current line if `unindentToLineIndentation` is
  `true`.
...
with
  ...
  @[computed_field] emptiness : Doc → Doc.Emptiness
    | .failure => .sometimesNonEmpty
    | .newline f? =>
      match f? with
      | none => .sometimesNonEmpty
      | some f =>
        if f.isEmpty then
          .alwaysEmptyIfFlattened
        else
...
-/
def Doc.isAlwaysEmpty (d : Doc) : Bool :=
  d.emptiness matches .alwaysEmpty

/--
Yields an empty document that serves as a neural element for `Doc.append`.
...
newlines.
-/
def Doc.fillUsingSpace (ds : Array Doc) : Doc := Id.run do
  if ds.size == 0 then
    return .empty
  let hd := ds[0]!
  if ds.size == 1 then
